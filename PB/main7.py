...
Y = train_targets['6'].values
Y = np.reshape(Y, [len(Y), 1])
X = torch.from_numpy(X)
Y = torch.from_numpy(Y)

val_x = val_features.values
val_y = val_targets['6'].values
...
cost = cost.cuda()
optimizer = torch.optim.Adam(model.parameters(), lr = 0.0005, weight_decay=0.0009)
losses = []
val_losses = []
min_loss = 100
min_epoch = 0
...
for i in range(800):
    ...
    for start in range(0, X.shape[0], batch_size):
        ...
        # rmse_t = torch.sqrt(loss)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        batch_loss.append(loss.data.cpu().numpy())
        # batch_loss.append(rmse_t.data.cpu().numpy())
...
ax.plot(losses, label='train loss', linestyle='-')
...
plt.plot(epochs, losses[:500], color='b', label='train loss')
plt.plot(epochs, val_losses[:500], color='y', label='val loss')
plt.legend(prop={'size': 15})
plt.ylabel('loss', fontname='Arial Narrow', size=20)
plt.xlabel('epoch', fontname='Arial Narrow', size=20)
plt.savefig('image/PRN_direct_loss.png', dpi=450)
plt.show()

#测试
y = test_targets['6'].values
y2 = test_targets['6'].values
...
plt.ylabel('PRN Prediction', fontname='Arial Narrow', size=20)
...
plt.title('Direct band gap', fontsize = 20, fontweight='heavy')
plt.text(6, 0, 'RMSE = 0.572\nr = 0.908')
plt.scatter(y[:], predict2, c='b', marker='o')

# plt.legend(loc='upper left',ncol=1, frameon=True, prop={'family':'Arial narrow','size':24})
plt.tight_layout()
plt.savefig('image/PRN_direct.png', dpi=450)
plt.show()